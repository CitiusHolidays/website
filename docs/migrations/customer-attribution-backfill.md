...
`customerJourneyEntitlements.by_authUserId_createdAt` lookup and cursor pagination. The staged
`clients.by_emailNormalized`, `inboundQueryIntents.by_contactEmailNormalized`, and
`queries.by_clientId` indexes are not reader dependencies and must not be activated merely to make
Account journeys available.
...
2. Deploy and finish any required staged-index backfill before activating a reader that explicitly
   depends on those indexes.
