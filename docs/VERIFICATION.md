...
| Local release evidence | `bun run verify:local` | The complete target-neutral release gate passes for the current working tree/revision. | Convex codegen against a selected deployment, a Next production build, Vercel state, or authenticated production behavior. |
| Hosted credential-free evidence | GitHub `Hosted Quality` for the exact revision | The same pinned-Bun target-neutral command as local verification ran from a clean clone, including diff hygiene, full tests, zero-warning lint, types, assets, audits, Studio, and performance-baseline freshness. | Target-aware Convex codegen, a deployment, migration, fresh performance measurement, or authenticated browser proof. |
