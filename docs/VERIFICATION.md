...
| Hosted credential-free evidence | GitHub `Hosted Quality` for the exact revision | The full isolated target-neutral test suite plus clean-clone-safe repository/Studio lint, asset, and dependency checks ran without secrets. | Generated Convex types, a deployment, migration, or authenticated browser proof. |
