  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex h-full flex-col justify-between">
            ...
            {heroLines.map((line, index) => (
              <span key={typeof line === "string" ? line : `${line.highlight}-${line.rest}`}>
                {typeof line === "string" ? (
                  ...
                  <>
                    <span className="text-auth-accent-on-dark italic">{line.highlight}</span>
                    {line.rest}
                  </>
                )}
        ...
        <m.div animate={{ opacity: 1 }} initial={{ opacity: 0 }} transition={{ delay: 0.6 }}>
          <AuthFeatureList
            items={copy.highlights.map((highlight, index) => ({
              ...highlight,
