...
export default function PilgrimageRouteFitSelector() {
  ...
  return (
    <section
      ...
      className="scroll-mt-24 border-brand-light border-y bg-public-paper py-10 md:py-16"
      id="journey-details"
    >
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto mb-10 max-w-3xl text-center">
          ...
          >
            Compare the two Kailash programmes
          </h2>
          <p className="text-base text-public-muted leading-relaxed md:text-lg">
            Select a published programme to view its itinerary or discuss it with a specialist.
        ...
        <fieldset>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {options.map((option) => (
              <label className="relative block cursor-pointer" key={option.slug}>
                <input
                  checked={selectedSlug === option.slug}
                  className="peer absolute top-7 left-6 size-4 accent-public-blue"
                ...
                <span className="flex min-h-44 flex-col rounded-2xl border border-brand-light bg-white p-6 shadow-sm transition-[border-color,box-shadow] peer-checked:border-citius-blue peer-checked:shadow-md peer-focus-visible:outline-2 peer-focus-visible:outline-citius-blue peer-focus-visible:outline-offset-4">
                  <span className="mb-4 pl-7 font-heading text-citius-blue text-xl">
                    {option.title}
                  </span>
                  ...
                  <span className="mb-2 text-brand-muted text-sm leading-relaxed">
                    <strong className="font-semibold text-brand-dark">Route:</strong> {option.route}
                  </span>
                  <span className="mb-2 text-brand-muted text-sm leading-relaxed">
                    <strong className="font-semibold text-brand-dark">Effort:</strong>{" "}
                    {option.difficulty}
                  </span>
                  <span className="mb-2 text-brand-muted text-sm leading-relaxed">
                    <strong className="font-semibold text-brand-dark">Altitude:</strong>{" "}
                    {option.maxAltitude}
                  </span>
                  <span className="text-brand-muted text-sm leading-relaxed">
                    <strong className="font-semibold text-brand-dark">Group:</strong>{" "}
                    {option.groupSize}
                  </span>
                </span>
              </label>
            ...
            <div className="flex flex-col items-start justify-between gap-5 md:flex-row md:items-center">
              <div>
                ...
                <p className="mt-1 text-public-muted text-sm">
                  Your enquiry includes this programme. You can edit it before sending.
                </p>
              </div>
