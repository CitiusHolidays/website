...
import { Columns3, RotateCcw, Save, Trash2 } from "lucide-react";
...
export function PortalTableCommandDock<Row>({
  ...
  const deleteWithStableFocusOrigin = async (
    ...
    await layoutContext?.deleteLayoutPreset(preset, optionsTriggerRef.current);
  };

  const renderOptionsTrigger = (props: ComponentProps<"button">) => (
    <button
      {...props}
      aria-label={`${ariaLabel}: Table options`}
  ...
  return (
    ...
    >
      <PortalActionMenu
        align="right"
        aria-label={`${ariaLabel}: Table options`}
        contentClassName="flex min-h-0 flex-col gap-1 p-2"
        header={
          <div>
            <p
              aria-live="polite"
              className="break-words font-medium text-brand-dark text-sm"
            ...
            <p className="mt-1 text-brand-muted text-xs">
              Layouts change columns and sort only. Identity and actions always remain.
            </p>
          </div>
        }
        headerClassName="shrink-0 border-brand-border border-b px-4 py-3"
        menuClassName="flex flex-col"
        onOpenChange={setOptionsMenuOpen}
        open={optionsMenuOpen}
        trigger={renderOptionsTrigger}
      ...
      >
                ...
                items.push(
                  <PortalActionMenuItem key={`${preset.id}:apply`} label={preset.name}>
                    <button
                      ...
                      type="button"
                    >
                      <span className="min-w-0 break-words">Apply {preset.name}</span>
        ...
        {layoutContext && currentLayout ? (
          <PortalActionMenuItem label="Save layout">
            <button
              ...
              onClick={() => {
                ...
                layoutContext.requestSaveLayout(currentLayout);
              }}
              type="button"
            >
        ...
        <PortalActionMenuItem disabled={!canReset} label="Reset layout">
          <button
            aria-label={`${ariaLabel}: Reset layout`}
            className="flex min-h-11 w-full items-center gap-2 rounded-xl px-3 text-left text-brand-dark text-sm hover:bg-brand-light aria-disabled:cursor-not-allowed aria-disabled:opacity-50"
            disabled={!canReset}
            onClick={() => {
              onReset();
              if (scope) {
