...
async function runInboundConversion({ args, convert, onComplete, onSuccess }) {
  try {
...
function useInboundLeadsController({ allowed, canFetch }) {
  ...
  const selectedId = selection.deepLinkedId === deepLinkedId ? selection.selectedId : deepLinkedId;
  const setSelectedId = (nextSelectedId) =>
    setSelection({ deepLinkedId, selectedId: nextSelectedId });
  ...
  const detailIsCurrent = form.intentId === (selected?._id ?? null);
  const currentForm = detailIsCurrent ? form : formFromIntent(selected);
  const currentDismissalReason = detailIsCurrent ? dismissalReason : "not_qualified";
  const currentError = detailIsCurrent ? error : "";
  const currentMessage = detailIsCurrent ? message : "";
  const notesWordCount = countWords(currentForm.notes);
  ...
  const setField = (field, value) =>
    setForm((current) => ({
      ...(current.intentId === (selected?._id ?? null) ? current : formFromIntent(selected)),
      [field]: value,
    }));
  const handleFieldChange = (event) =>
    setField(event.currentTarget.name, event.currentTarget.value);
  const handleSearchChange = (event) => setSearch(event.currentTarget.value);
  const handleQueryTypeChange = (value) => setField("queryType", value);
  const handleTravelTypeChange = (value) => setField("travelType", value);
  const handleDismissalReasonChange = (value) => {
    if (!detailIsCurrent) {
      setForm(formFromIntent(selected));
    }
    setDismissalReason(value);
  };
  const loadMore = () => page.loadMore(50);
  ...
  const handleConvert = async (event) => {
    ...
    const conversionError = await runInboundConversion({
      args: {
        ...currentForm,
        budgetAmount: currentForm.budgetAmount ? Number(currentForm.budgetAmount) : undefined,
        intentId: selectedId,
        paxCount: Number(currentForm.paxCount),
      },
      convert,
  ...
  const handleDismiss = async () => {
    ...
    setMessage("");
    try {
      await dismiss({ dismissalReason: currentDismissalReason, intentId: selectedId });
  ...
  return {
    dismissalReason: currentDismissalReason,
    error: currentError,
    form: currentForm,
    handleConvert,
    handleDismiss,
    ...
    selected,
    selectedId,
    setDismissalReason: handleDismissalReasonChange,
    setSelectedId,
    setSource,
    setStatus,
    shouldFetch,
