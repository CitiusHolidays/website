...
function EditionUnderReview() {
  return (
    <main className="flex min-h-[100svh] items-center bg-public-night px-5 py-16 text-white">
      <div className="mx-auto w-full max-w-xl rounded-[2rem] border border-white/10 bg-white/[0.055] p-7 shadow-[0_28px_100px_rgb(0_0_0_/_0.3)] sm:p-10">
        <p className="font-semibold text-public-orange text-xs uppercase tracking-[0.18em]">
          Sacred Bharat
        </p>
        <h1 className="mt-4 font-heading text-4xl text-public-paper leading-tight sm:text-5xl">
          This edition is under review.
        </h1>
        <p className="mt-4 text-base text-white/70 leading-7">
          Sacred Bharat / 001 is temporarily unavailable while its content is reviewed or updated.
          No quiz or sharing data is being collected from this page.
        </p>
...
export default async function SacredBharatEditionPage() {
  const availability = await getEditionAvailability();
