...
const billingPolicy = {
  content: [
    ...
    {
      footer:
        "All online transactions are processed securely through our integrated payment gateway partner.",
    ...
    {
      footer:
        "If the balance is not paid by the due date, Citius may release bookings without refund of the advance.",
    ...
    {
      footer:
        "Please share proof of transaction (screenshot / reference number). Our team will coordinate with the payment gateway; resolution may take 3–5 working days.",
...
const termsPolicy = {
  content: [
    ...
    {
      footer:
        "No refund is provided for: No-show, Unused services, Early check-out, Missed tours due to delays or personal reasons",
  ...
  intro:
    "Welcome to Citius Holidays. By accessing our website, booking a service, or engaging with our travel programs, you agree to the following Terms & Conditions. Please read them carefully before proceeding.",
            ...
            <ul className="space-y-2">
              {sub.list.map((item) => (
                <li className="flex items-start gap-2 text-gray-700 text-sm" key={item}>
                  <span className="mt-1.5 size-1.5 shrink-0 rounded-full bg-citius-orange" />
                  <span>{item}</span>
                </li>
              ))}
            </ul>
      ...
      <ul className="mb-4 space-y-3">
        {data.list.map((item) => (
          <li className="group flex items-start gap-3" key={item}>
            <span className="mt-1.5 size-1.5 shrink-0 rounded-full bg-citius-blue transition-colors duration-300 group-hover:bg-citius-orange" />
            <span className="font-content text-gray-700 transition-colors duration-300 group-hover:text-gray-900">
              {item}
            </span>
          </li>
        ))}
...
export default function PolicyContent() {
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7]">
      ...
      {/* Progress Bar */}
      <m.div
        className="fixed top-0 right-0 left-0 z-50 h-1 origin-left bg-citius-orange"
        style={{ scaleX }}
      />
      {/* Header */}
      <header className="relative overflow-hidden bg-brand-dark px-6 pt-32 pb-20 text-white">
        <div className="absolute inset-0 bg-[url('/gallery/bgaboutus.webp')] bg-center bg-cover opacity-20 mix-blend-overlay" />
        <div className="absolute inset-0 bg-gradient-to-b from-brand-dark/0 via-brand-dark/50 to-brand-dark" />

        <div className="relative z-10 mx-auto max-w-4xl text-center">
          <m.h1
            animate={{ opacity: 1, y: 0 }}
            className="mb-6 font-heading text-4xl md:text-6xl"
          ...
          <m.p
            animate={{ opacity: 1 }}
            className="mx-auto max-w-xl font-sans text-gray-400 text-xl"
            initial={{ opacity: 0 }}
          ...
          <div className="flex justify-center gap-8 md:gap-16">
            {[
              ...
              >
                ...
                {activeTab === tab.id && (
                  <m.div
                    className="absolute right-0 bottom-0 left-0 h-0.5 bg-citius-orange"
                    layoutId="activeTab"
          ...
          >
            {activeTab === "billing" ? (
              <div className="space-y-8">
                <div className="mb-8 border-gray-200 border-b pb-8">
                ...
                {billingPolicy.content.map((section) => (
                  <Section data={section} key={section.heading} />
                ))}
              </div>
            ) : (
              <div className="space-y-8">
                <div className="mb-8 border-gray-200 border-b pb-8">
                  ...
                  <p className="mb-6 font-sans text-gray-500 text-sm uppercase tracking-widest">
