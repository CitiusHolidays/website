...
export function parseExcelDateCode(serial, opts, b2) {
  if (typeof serial !== "number" || !Number.isFinite(serial) || serial > 2958465 || serial < 0) {
    return null;
  }
  let date = serial | 0;
  let time = Math.floor(86400 * (serial - date));
  let dow = 0;
  const out = {
    D: date,
    T: time,
    u: 86400 * (serial - date) - time,
    y: 0,
    m: 0,
    d: 0,
    H: 0,
...
function normalizeCellValue(value) {
  ...
  if (typeof value === "object") {
    if (Object.hasOwn(value, "result")) return normalizeCellValue(value.result);
